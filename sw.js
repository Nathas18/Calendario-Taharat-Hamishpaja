const CACHE='luaj-tahara-pwa-v8';
const CORE=[
  './',
  './index.html',
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(fetch(request).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(request,copy));}return response;}).catch(()=>caches.match(request)));
});
